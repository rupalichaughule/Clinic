      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
              <li class=""><a class="btn btn-primary" href="/addappointment">Book New Appointment</a> &nbsp;&nbsp;</li>
          ...
          <div class="card">
            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  @php $x=1; @endphp
                @foreach($appointments as $appointmentsnew)
                <tr>
                  <td>{{$x++}}</td>
                  
                  <td>{{$appointmentsnew->name}}</td>
                 
                  <td>{{$appointmentsnew->phone}}</td>
                  <td>{{$appointmentsnew->email}}</td>
                  <td>{{$appointmentsnew->service}}</td>
                  <td>{{$appointmentsnew->date}}</td>
                  <td>{{$appointmentsnew->time}}</td>
                  <td>{{$appointmentsnew->doctorname}}</td>
                  <td>{{$appointmentsnew->message}}</td>
                  
                  <td>
                    
                    <a class="btn btn-warning btn-sm" href="/editappointment/{{$appointmentsnew->id}}"><i class="fa fa-edit"></i></a>
                    <a class="btn btn-danger btn-sm" href="#" onclick="confirm_doctordelete_modal('/deleteuser/{{$appointmentsnew->id}}');"><i class="fa fa-trash"></i></a>
                    
                    @if($appointmentsnew->status=='0') 
                          <div class="btn-group mr-2 mb-2" data-toggle="tooltip" data-animation="false" data-original-title="Change Status">
                              ...
                              <div style="padding: 0;min-width: 60px;margin: 0;" aria-labelledby="dropdownMenuButton4" class="dropdown-menu">
                                  <a class="btn btn-success btn-sm lighten dropdown-item" onclick="active_modal('/activeappointmentstatus/{{$appointmentsnew->id}}');" href="#"> Active</a>
                              </div>
                          </div>
                          @else
                          <div class="btn-group mr-2 mb-2" data-toggle="tooltip" data-animation="false" data-original-title="Change Status">
                              ...
                              <div style="padding: 0;min-width: 60px;margin: 0;" aria-labelledby="dropdownMenuButton4" class="dropdown-menu">
                                  <a class="btn btn-danger btn-sm lighten dropdown-item" onclick="inactive_modal('/inactiveappointmentstatus/{{$appointmentsnew->id}}');" href="#"> In-active</a>
                              </div>
                          </div>
                          @endif
      ...
      <div class="modal-content">
          <div class="modal-body p-4">
              <div class="text-center">
                  ...
                  <h4 class="mt-2">Delete Appointment!</h4>
