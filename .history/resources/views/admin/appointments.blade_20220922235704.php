            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td>{{$doctorsnew->name}}</td>
                 
                  <td>{{$doctorsnew->phone}}</td>
                  <td>{{$doctorsnew->email}}</td>
                  <td>{{$doctorsnew->service}}</td>
                  <td>{{$doctorsnew->date}}</td>
                  <td>{{$doctorsnew->time}}</td>
                  <td>{{$doctorsnew->doctor}}</td>
                  <td>{{$doctorsnew->message}}</td>
                  
                  <td>
                    
                    <a class="btn btn-warning btn-sm" href="/editappointment/{{$doctorsnew->id}}"><i class="fa fa-edit"></i></a>
                    <a class="btn btn-danger btn-sm" href="#" onclick="confirm_doctordelete_modal('/deleteuser/{{$doctorsnew->id}}');"><i class="fa fa-trash"></i></a>
                    
                    @if($doctorsnew->status=='0') 
                          <div class="btn-group mr-2 mb-2" data-toggle="tooltip" data-animation="false" data-original-title="Change Status">
                              ...
                              <div style="padding: 0;min-width: 60px;margin: 0;" aria-labelledby="dropdownMenuButton4" class="dropdown-menu">
                                  <a class="btn btn-success btn-sm lighten dropdown-item" onclick="active_modal('/activestatus/{{$doctorsnew->id}}');" href="#"> Active</a>
                              </div>
                          </div>
                          @else
                          <div class="btn-group mr-2 mb-2" data-toggle="tooltip" data-animation="false" data-original-title="Change Status">
                              ...
                              <div style="padding: 0;min-width: 60px;margin: 0;" aria-labelledby="dropdownMenuButton4" class="dropdown-menu">
                                  <a class="btn btn-danger btn-sm lighten dropdown-item" onclick="inactive_modal('/inactivestatus/{{$doctorsnew->id}}');" href="#"> In-active</a>
                              </div>
                          </div>
                          @endif
