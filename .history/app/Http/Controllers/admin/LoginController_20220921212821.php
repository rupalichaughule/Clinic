    ...
    function checklogin(Request $request)
    {
     ...
     if(count($userdata)>0) // check query result
     {
         if(!isset($_SESSION)) //if session is not started
         {
            ...
            $_SESSION['adminid'] = $userdata[0]->id; // store query result in session
            $_SESSION['adminname'] = $userdata[0]->name;
            $_SESSION['adminrole'] = $userdata[0]->role;
            $_SESSION['adminphone'] = $userdata[0]->phone;
            $_SESSION['adminemail'] = $userdata[0]->email;
            $_SESSION['adminstatus'] = $userdata[0]->status;
