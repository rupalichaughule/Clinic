...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $id='1';
        $websitedata=Websitecontent::find($id);
        $ads=Ad::where('status','1')->orderBy('id','Desc')->get();
        if(isset($_SESSION['userid'])){
            $data= User::find($_SESSION['userid']);
            $usercity=$data->district;
            $myads=Ad::whereIn('locations',[$usercity])->orderBy('id','Desc')->get();
        }else{
            $usercity='';
            $myads=0;
        }
        // return $myads;
        // $sources= DB::table('cities')->select('id','name')->get();
        ...
        $listings=DB::table('listings')
            ...
            ->where('listings.status','1')->orderBy('listings.id','DESC')->limit(10)->get();
       
        $vehicletypes=Vehicletype::orderBy('id')->get();
        $packages=Package::get();
        return view('webapp.index',compact('banners','websitedata','listings','vehicletypes','ads','sources','packages','usercity','myads'));
    }
    public function register()
    {
        $users=User::get();
        $sources=Source::orderBy('id')->get();
        return view('webapp.register',compact('users','sources'));
    }
     ...
     function resendotp()
    {
            ...
            // $ch = curl_init();
            // curl_setopt($ch,CURLOPT_URL, "http://sms.mavericinfotech.in/app/smsapi/index.php");
            ...
            // MAveric SMS API Start
             ...
             $curl = curl_init();
                ...
                curl_setopt_array($curl, array(
                CURLOPT_URL => "http://2factor.in/API/V1/".$settings->sms_gateway_key."/SMS/".$phone."/".$rndno,
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => "",
        ...
        // $checksponserid= User::where('userid',$request->email)->get();
           ...
           $data= new User();
            ...
            //     curl_close($curl);
                $data->otp=$rndno;
                $data->save();
            return redirect('/verify-otp/'.$data->id);
    ...
    public function mylocation($id)
    {
        $user=User::find($id);
        // $sources=Source::orderBy('id')->get();
        $ip=\Request::ip();
        $data = Location::get($ip);
        return view('webapp.my-location',compact('user','data'));
    }
    function logout()
    {
        if(!isset($_SESSION)) 
